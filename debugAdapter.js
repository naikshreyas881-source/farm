    ...
    (function(root, factory) {
      ...
      Object.defineProperty(ES5Class, "$create", {
        value: function $create() {
          var self2 = this, instance = self2.$apply.length ? /* @__PURE__ */ Object.create(null) : Object.create(self2.prototype), args = BetterCurry.flatten(arguments);
            ...
            if ((msg.hasOwnProperty("result") || msg.hasOwnProperty("error")) && msg.hasOwnProperty("id")) {
              ...
              this.endpoint.handleCall(msg, this, function handleCall(err, result) {
                if (err) {
                  ...
                  EventEmitter2.trace(
                    "-->",
                    "Failure " + (EventEmitter2.hasId(msg) ? "(id " + msg.id + ")" : "") + ": " + (err.stack ? err.stack : err.toString())
                  );
                }
...
module.exports = __toCommonJS(goDebug_exports);
var import_child_process = require("child_process");
var import_events = require("events");
var fs3 = __toESM(require("fs"));
var import_fs = require("fs");
var glob = __toESM(require_glob());
var import_json_rpc2 = __toESM(require_jsonrpc());
...
function killProcessTree(p, logger2 = console.log) {
...
var Delve = class {
  ...
  loadConfig;
  connection;
  // null if connection isn't necessary (e.g. noDebug mode)
  onstdout;
  onstderr;
  onclose;
  noDebug;
  ...
  constructor(launchArgs, program) {
    ...
    this.connection = new Promise(async (resolve2, reject) => {
      ...
      if (launchArgs.request === "launch") {
        ...
        if (!(0, import_fs.existsSync)(launchArgs.dlvToolPath)) {
          log(
            `Couldn't find dlv at the Go tools path, ${process.env["GOPATH"]}${env["GOPATH"] ? ", " + env["GOPATH"] : ""} or ${getEnvPath()}`
          );
          return reject(
      ...
      function connectClient(port, host, onClose) {
        setTimeout(() => {
          const conn = import_json_rpc2.Client.$create(port, host).connectSocket();
          conn.on("connect", () => resolve2(conn)).on("error", reject).on("close", (hadError) => {
   ...
   */
  async getDebugState() {
    const callResult = await this.callPromise("State", [{ NonBlocking: true }]);
  ...
  async close() {
    ...
    return new Promise(async (resolve2) => {
      ...
      const timeoutToken = isLocalDebugging && setTimeout(async () => {
...
var GoDebugSession = class extends import_vscode_debugadapter.LoggingDebugSession {
  ...
  stopOnEntry;
  logLevel = import_vscode_debugadapter.Logger.LogLevel.Error;
  initdone = "initdone\xB7";
  remoteSourcesAndPackages = new RemoteSourcesAndPackages();
  localToRemotePathMapping = /* @__PURE__ */ new Map();
  remoteToLocalPathMapping = /* @__PURE__ */ new Map();
  // TODO(suzmue): Use delve's implementation of substitute-path.
  substitutePath;
  showGlobalVariables = false;
  continueEpoch = 0;
  continueRequestRunning = false;
  nextEpoch = 0;
  ...
  async disconnectRequestHelper(response, args) {
  ...
  inferLocalPathInGoRootFromRemoteGoPackage(remotePathWithLocalSeparator, relativeRemotePath) {
    ...
    const localGoRootImportPath = path2.join(
      goroot,
      srcIndex >= 0 ? remotePathWithLocalSeparator.substr(srcIndex) : path2.join("src", relativeRemotePath)
    );
  ...
  inferLocalPathInGoPathFromRemoteGoPackage(remotePathWithLocalSeparator, relativeRemotePath) {
    ...
    const localGoPathImportPath = path2.join(
      gopath,
      indexGoModCache >= 0 ? remotePathWithLocalSeparator.substr(indexGoModCache) : path2.join("pkg", "mod", relativeRemotePath)
    );
    if (this.fileSystem.existsSync(localGoPathImportPath)) {
      return localGoPathImportPath;
    }
    const localGoPathSrcPath = path2.join(
      gopath,
      "src",
      ...
      this.delve?.callPromise("Command", [{ name: "halt" }]).then(
        () => {
          return this.setBreakPoints(response, args).then(() => {
            ...
            return this.continue(true).then(void 0, (err) => {
  ...
  async threadsRequest(response) {
    ...
    this.delve?.call("ListGoroutines", [], (err, out) => {
      ...
      const threads = goroutines.map(
        (goroutine) => new import_vscode_debugadapter.Thread(
          goroutine.id,
    ...
    this.delve?.call(
      ...
      async (err, out) => {
        ...
        let stackFrames = locations.map((location, frameId) => {
          ...
          return new import_vscode_debugadapter.StackFrame(
            uniqueStackFrameId,
            location.function ? location.function.name : "<unknown>",
            location.file === "<autogenerated>" ? void 0 : new import_vscode_debugadapter.Source(path2.basename(location.file), this.toLocalPath(location.file)),
            location.line,
            0
          );
        });
  ...
  scopesRequest(response, args) {
    ...
    this.delve?.call(
      ...
      (err, out) => {
        ...
        this.delve?.call(
          "ListFunctionArgs",
          this.delve?.isApiV1 ? [listLocalFunctionArgsIn] : [{ scope: listLocalFunctionArgsIn, cfg: this.delve?.loadConfig }],
          (listFunctionErr, outArgs) => {
            ...
            vars.push(...locals);
            const shadowedVars = /* @__PURE__ */ new Map();
            for (let i = 0; i < vars.length; ++i) {
              if ((vars[i].flags & 2 /* VariableShadowed */) === 0) {
                continue;
              }
            ...
            this.getPackageInfo(this.debugState).then((packageName) => {
              ...
              this.delve?.call(
                ...
                (listPkgVarsErr, listPkgVarsOut) => {
                  ...
                  scopes.push(
                    new import_vscode_debugadapter.Scope("Global", this.variableHandles.create(globalVariables), false)
                  );
                  this.sendResponse(response);
                  log("ScopesResponse");
                }
  ...
  variablesRequest(response, args) {
    ...
    const loadChildren = async (exp, v) => {
      if (v.kind === 25 /* Struct */ && v.len > v.children.length || v.kind === 20 /* Interface */ && v.children.length > 0 && v.children[0].onlyAddr === true) {
    ...
    if (vari.kind === 17 /* Array */ || vari.kind === 23 /* Slice */) {
      ...
      variablesPromise = Promise.all(
        ...
        // remove the null values created by combining keys and values
      );
    } else {
      variablesPromise = Promise.all(
        vari.children.map((v) => {
      ...
      this.evaluateCallImpl(args).then(
        ...
        (err) => {
          this.sendErrorResponse(
            ...
            args.context === "watch" ? void 0 : import_vscode_debugadapter.ErrorDestination.User
          );
        }
      );
      return;
   ...
   */
  async initializeRemotePackagesAndSources() {
  ...
  async setBreakPoints(response, args) {
          ...
          return this.delve?.callPromise("CreateBreakpoint", [
            ...
            if (err.toString().startsWith("Breakpoint exists at")) {
              ...
              const matchedBreakpoint = existingBreakpoints.find(
                (existingBreakpoint) => existingBreakpoint.line === breakpointIn.line && compareFilePathIgnoreSeparator(existingBreakpoint.file, breakpointIn.file)
              );
  ...
  async getPackageInfo(debugState) {
    ...
    const dir = path2.dirname(
      this.delve?.remotePath?.length || this.delve?.isRemoteDebugging ? this.toLocalPath(debugState.currentThread.file) : debugState.currentThread.file
    );
  ...
  wrapReturnVars(vars) {
    ...
    return {
      value: values.join(", "),
      kind: 0 /* Invalid */,
      flags: 32 /* VariableFakeAddress */ | 16 /* VariableReturnArgument */,
      children: vars,
  ...
  convertDebugVariableToProtocolVariable(v) {
    if (v.kind === 26 /* UnsafePointer */) {
    ...
    } else if (v.kind === 17 /* Array */) {
      return {
        result: "<" + v.type + ">",
        variablesReference: this.variableHandles.create(v)
      };
    } else if (v.kind === 24 /* String */) {
      let val = v.value;
      ...
      if (child.kind === 0 /* Invalid */ && child.addr === 0) {
        return {
          result: `nil <${v.type}>`,
          variablesReference: 0
        };
  ...
  evaluateRequestImpl(args) {
    ...
    const returnValue = this.delve?.callPromise(this.delve?.isApiV1 ? "EvalSymbol" : "Eval", [evalSymbolArgs]).then(
      (val) => val,
      (err) => {
        log(
...
var _RemoteSourcesAndPackages = class extends import_events.EventEmitter {
  initializingRemoteSourceFiles = false;
  initializedRemoteSourceFiles = false;
  remotePackagesBuildInfo = [];
  remoteSourceFiles = [];
  remoteSourceFilesNameGrouping = /* @__PURE__ */ new Map();
  /**
   ...
   */
  async initializeRemotePackagesAndSources(delve) {
    this.initializingRemoteSourceFiles = true;
    try {
      if (!delve.isApiV1 && this.remotePackagesBuildInfo.length === 0) {
        const packagesBuildInfoResponse = await delve.callPromise(
      ...
      if (delve.isApiV1) {
        ...
        const listSourcesResponse = await delve.callPromise("ListSources", [{}]);
      ...
      logError(`Failed to initialize remote sources and packages: ${error.message}`);
    } finally {
...
__publicField(RemoteSourcesAndPackages, "INITIALIZED", "INITIALIZED");
function random(low, high) {
  return Math.floor(Math.random() * (high - low) + low);
}
async function removeFile(filePath) {
  try {
    const fileExists = await fsAccess(filePath).then(() => true).catch(() => false);
